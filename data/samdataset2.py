...
def get_sam_transforms(size=1024):
    """
    SAM preprocessing:
    - samo resize, bez normalize ili ToTensorV2
...
class RetinaDatasetSAM(Dataset):
    ...
    - Vraća slike u uint8 RGB formatu
    - Maske i FOV su uint8 (0/1)
    - Čuva original_size za vraćanje predikcija nazad
    """
    ...
    def __getitem__(self, idx):
        # Učitaj raw slike i maske u numpy
        ...
        # Albumentations transform radi na numpy
        ...
        return {
            "image": image,                 # numpy uint8 (H,W,3)
            "mask": mask,                   # numpy uint8 (H,W)
            "fov": fov,                     # numpy uint8 (H,W) ili None
