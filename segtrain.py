...
def unnormalize(imgs, mean, std):
    """ 
        Napomena:
        Normalizaciju je potrebno primijeniti tokom treninga.
        Nakon toga, radi vizualizacije, slike treba vratiti u originalni opseg
    """
...
def train_model(data_dir, epochs, batch_size, lr, device='cuda', log_dir=None):
    ...
    def log_images(images, masks, preds, epoch, tag: str = "Validation"):
        ...
        def _prep(t: torch.Tensor) -> torch.Tensor:
            ...
            if t.size(1) == 1:
                t = t.repeat(1, 3, 1, 1)
            return t.float()
        grid = make_grid(torch.cat([_prep(images), _prep(masks), _prep(preds)], dim=0), nrow=images.size(0))
    ...
    cur_bs, cur_size = batch_size, image_size
    while True:
        try:
            attempt_training(cur_bs)
            break
        except RuntimeError as e:
            if "CUDA out of memory" in str(e):
                print(f"\n⚠️  OOM at bs={cur_bs}, img={image_size}. Retrying...")
                torch.cuda.empty_cache()
                if cur_bs > 1:
                    cur_bs = max(1, cur_bs // 2)
                else:
                    raise RuntimeError("Cannot recover from OOM: batch_size=1 and image_size=512 failed")
                print(f"🔁 New attempt: bs={cur_bs}, img={image_size}")
            else:
                raise
